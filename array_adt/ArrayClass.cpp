...
Array *Array::Merge(Array arr2) {
  int i, j, k = 0;
  Array *mereged_array = new Array(length + arr2.length);
  while (i < length || j < arr2.length) {
    if (A[i] < arr2.A[j]) {
      mereged_array->A[k++] = A[i];
      i++;
    } else {
      mereged_array->A[k++] = arr2.A[j];
      j++;
    }
  }
  if (i >= length) {
    for (j = j; j < arr2.length; j++) {
      mereged_array->A[j] = arr2.A[j];
    }
  } else {
    for (k = k; k < length; k++) {
      mereged_array->A[k] = A[k];
    }
  }
  mereged_array->length = length + arr2.length;
